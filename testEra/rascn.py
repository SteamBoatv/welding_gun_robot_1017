...
def extract_change_points(point_cloud, threshold_angle, knn):  # 点云集合 阈值角度 搜索范围
    ...
    for i in range(1, num_points):
        ...
        dot_products = np.dot(neighbor_normals, cc)
        # 绝对值
        dot_products = np.abs(dot_products)
        # 归一化
        dot_products /= np.linalg.norm(dot_products)
        FLAG = True
        for dot_product in dot_products:
            if dot_product > threshold_angle:
                FLAG = False
        if FLAG:
...
def fit_intersection_line(hanfeng_point):
    ...
    # ----------------提取焊缝-------------------------------------
    hanfeng = extract_change_points(hanfeng_point, 0.34, 9)  # 判断突变 阈值 搜索半径
...
def hanfengtiqu(point_list, str):
    ...
    for i in range(len):
        [k2, idx2, _] = pcd_tree2.search_radius_vector_3d(plane1.points[i], 0.01)
        if k2 > 10:
